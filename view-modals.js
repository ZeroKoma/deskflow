  ...
  container.querySelectorAll(".tag-chip").forEach((chip) => {
    if (!chip.classList.contains("protected-tag")) {
      chip.addEventListener("click", () => {
        ...
        chip.classList.toggle("selected"); chip.classList.toggle("inactive");
      });
    }
  });
}
...
export function renderTagManager() {
  ...
  container.innerHTML = state.tags.map((tag) => {
      const isProtected = tag.name === "Alarma" || tag.id === "tag-alarm-default";
      return `
      <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px; border-bottom: 1px solid var(--border);">
        ...
        ${isProtected ? `<span style="font-size: 0.75rem; color: var(--text-muted); font-style: italic;"><i class="fas fa-lock"></i> Sistema</span>`
